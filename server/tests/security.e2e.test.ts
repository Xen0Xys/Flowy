...
describe("Security regression coverage (e2e)", () => {
    ...
    test("JWT with mismatched audience is rejected", async () => {
        const payload = buildRegisterPayload();
        const register = await agent.post("/auth/register").send(payload);
        expect(register.status).toBe(201);
        ...
        const token = buildHs512JwtToken({
            userId: dbUser.id,
            email: dbUser.email,
            jwtId: dbUser.jwt_id,
    ...
    test("issued authentication token includes expiry and does not use alg=none", async () => {
        const payload = buildRegisterPayload();
        const register = await agent.post("/auth/register").send(payload);
        expect(register.status).toBe(201);
        ...
        const jwtPayload = JSON.parse(Buffer.from(segments[1], "base64url").toString("utf8")) as {
            exp?: number;
            iat?: number;
        };
        ...
        expect((jwtPayload.exp ?? 0) > (jwtPayload.iat ?? 0)).toBe(true);
    });
...
function buildHs512JwtToken(input: {userId: string; email: string; jwtId: string; audience: string}): string {
    const secret = process.env.APP_SECRET;
    const issuer = process.env.APP_NAME;

    if (!secret || !issuer) {
        throw new Error("APP_SECRET and APP_NAME are required for JWT validation tests");
    }
