...
export class FamilyAdminGuard implements CanActivate {
    canActivate(context: ExecutionContext): boolean {
        ...
        if (!user.familyId) throw new ForbiddenException("User is not associated with a family");

        if (user.familyRole !== UserRoles.ADMIN) throw new ForbiddenException("User must be a family admin");
