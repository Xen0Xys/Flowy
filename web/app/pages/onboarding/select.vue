...
async function joinFamily() {
    ...
    try {
        ...
        error.value = err?.data?.message ?? err?.message ?? "Failed to join family";
    } finally {
        loading.value = false;
    }
}
...
<template>
    <main>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-top: 1rem;">
            <Card>
                <h2>Create</h2>
                <p>Create a new family to manage shared expenses and invite members. The creator becomes the family admin and can send invites.</p>
                <div>
            ...
            <Card>
                <h2>Join</h2>
                <p>If someone already created a family, enter the invite code they sent you to join. You will be added as a family member.</p>
                <div>
                    <Input v-model="code" placeholder="Invite code" />
                </div>
                <div style="margin-top: 0.5rem;">
                    <Button :as="'button'" :disabled="loading" @click="joinFamily">{{ loading ? 'Joining...' : 'Join' }}</Button>
                </div>
