...
154. CONVENTIONAL COMMITS — Use Conventional Commits (https://www.conventionalcommits.org). Allowed types: `feat`, `fix`, `docs`, `style`, `refactor`, `perf`, `test`, `build`, `ci`, `chore`, `revert`. Scope is optional but, when present, must be one of: `web`, `server`, `prisma`, `docker`, `ci`, `deps`, `docs`, `release`, `seed`, `security`, `i18n`, `auth`, `sso`, `mfa`, `users`, `admin`, `accounting`, `accounts`, `transactions`, `transfers`, `budgets`, `recurring`, `references`, `reports`, `updates`, `import`, `settings`, `onboarding`. Use `!` after the scope for breaking changes. Examples: `feat(sso): add OIDC callback`, `fix(reports): correct cash flow rounding`, `chore(deps): bump nuxt to 4.5.2`, `refactor(auth)!: replace jwt payload shape`. Existing gitmoji history stays as-is; the new convention applies to new commits only.
155. COMMIT FORMAT — Use `type(scope)?: subject` with subject in imperative mood and 100 chars max (Angular convention). Keep each commit focused on a single concern. The local `commit-msg` hook (installed by `bun install`) and the `commit-lint` CI job both enforce this format; use `git commit --no-verify` only in emergencies.
